...
This project pushed me past just wiring together AI APIs — it required curating a training set and evaluating the resulting model's real limitations, designing a retrieval pipeline that blends categorical tag-matching with semantic similarity, and building both a CLI and a Streamlit UI so the system was something I could actually use, not just read code for. The UI wasn't part of my original plan — after presenting the terminal-only version, I got feedback that it needed a real interface, so I went back and built one. Using my own system through an actual UI, instead of just reading the scoring code, is what surfaced a real bug: theme preferences barely changed the recommendations because they were weighted the same flat amount as much stronger signals. Catching that by clicking through my own product rather than re-reading code is the part of this project I'd point to first — it's the gap between code that looks correct and a system that behaves correctly, and you only find that gap by actually using the thing.
